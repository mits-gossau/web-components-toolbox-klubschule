...
export default class VotingController extends Shadow() {
  constructor (options = {}, ...args) {
    ...
    this.requestVotingData = (event) => {
      ...
      this.dispatchEvent(
        new CustomEvent('voting-data', {
          detail: {
            fetch: fetch(`${endpoint}/data`, fetchOptions).then(
              async (response) => {
    ...
    this.submitListener = (event) => {
      this.dispatchEvent(
        new CustomEvent('submit-voting-response', {
          detail: {
            fetch: fetch(`${endpoint}/voting`, {
              ...fetchOptions,
  ...
  connectedCallback () {
    this.addEventListener('request-voting-data', this.requestVotingData)
    this.addEventListener('submit-voting', this.submitListener)
  }

  disconnectedCallback () {
    this.removeEventListener('request-voting-data', this.requestVotingData)
    this.removeEventListener('submit-voting', this.submitListener)
  }
}
