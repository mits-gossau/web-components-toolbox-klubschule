...
export default class WishList extends Shadow() {
      ...
      timeoutIds.set(request, setTimeout(() => {
        ...
        this.dispatchEvent(new CustomEvent('with-facet', {
          detail: {
            ...
            fetch: (fetchPromise = wishListCache.has(request)
              ? wishListCache.get(request)
              // TODO: wishListCache key must include all variants as well as future payloads
              // TODO: know the api data change cycle and use timestamps if that would be shorter than the session life time
              : wishListCache.set(request, fetch(apiUrl, requestInit).then(response => {
                ...
                json.filters.forEach(filterItem => {
                  if (filterItem.children && filterItem.children.length > 0 && filterItem.visible) {
                    ...
                    WishList.historyPushState({}, '', `${this.url.origin}${this.url.pathname}?${this.params.toString()}`)
                  }
                })
  ...
  connectedCallback () {
    this.addEventListener('request-with-facet', this.requestWishListListener)
    this.addEventListener('reset-all-filters', this.requestWishListListener)
    this.addEventListener('reset-filter', this.requestWishListListener)
    this.addEventListener('request-locations', this.requestLocations)
    self.addEventListener('popstate', this.popstateListener)
  }
  
  disconnectedCallback () {
    this.removeEventListener('request-with-facet', this.requestWishListListener)
    this.removeEventListener('reset-all-filters', this.requestWishListListener)
    this.removeEventListener('reset-filter', this.requestWishListListener)
