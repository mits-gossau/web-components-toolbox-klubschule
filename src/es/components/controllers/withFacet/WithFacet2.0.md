...
- Initial request bootstrap
  ...
  - Legacy compatibility: if initialRequestObj.filter exists (old schema), interpret it as initialRequestObj.filters and drop filter. The API should accept both, normalize server-side, and persist the normalized form in the session.
