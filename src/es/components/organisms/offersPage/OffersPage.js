...
export default class OffersPage extends Shadow() {
  constructor(options = {}, ...args) {
    ...
    this.withFacetListener = (event) => {
      Promise.resolve(event.detail.fetch).then((data) => {
        ...
        } else if (this.partnerSearchWithResult) {
  ...
  renderHTML() {
    ...
    return this.fetchModules([
      ...
      {
        path: `${this.importMetaUrl}../../molecules/historyCompleteList/HistoryCompleteList.js`,
          ...
          : ''}
            ...
            `}
              ...
              <ks-m-tile-factory 
                ...
                ${this.eventDetailURL ? '' : `tracking-context="${this.getAttribute('tracking-context') || (this.hasAttribute('is-info-events') ? 'info_session' : this.hasAttribute('is-other-locations') ? 'other_locations' : this.isWishList ? 'wishlist_offers' : 'offers_list')}"`}
              >
                ${this.hiddenSectionsPartnerSearch.reduce((acc, hiddenSection) => (acc + hiddenSection.outerHTML), '')}
