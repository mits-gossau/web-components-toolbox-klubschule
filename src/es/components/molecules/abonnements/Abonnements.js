  ...
  renderContent(data) {
    this.fetchModules([
      ...
      data.courses.forEach((course) => {
        this.tiles.push(course.locations?.length ? /* html */ `<ks-o-tile-list data='${JSON.stringify(course).replace(/'/g, '’').replace(/"/g, '\"')}'></ks-o-tile-list>` : /* html */ `<ks-m-tile namespace="tile-default-" data='${JSON.stringify(course).replace(/'/g, '’').replace(/"/g, '\"')}'></ks-m-tile>`)
      })
