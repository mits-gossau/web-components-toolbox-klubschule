  ...
  renderContent(data) {
    this.fetchModules([
      ...
      data.courses.forEach((course) => {
        this.tiles.push(course.locations?.length > 1 || course.buttons[0].link === null &&  course.buttons[0].iconName === 'ChevronDown' &&  course.buttons[0].typ === 'quaternary' ? /* html */ `<ks-o-tile-list is-abo-list no-url-params${this.hasAttribute('tracking-context') ? ` tracking-context="${this.getAttribute('tracking-context')}"` : ''} data='${JSON.stringify(course).replace(/'/g, '’').replace(/"/g, '\"')}'></ks-o-tile-list>` : /* html */ `<ks-m-tile no-url-params namespace="tile-default-"${this.hasAttribute('tracking-context') ? ` tracking-context="${this.getAttribute('tracking-context')}"` : ''} data='${JSON.stringify(course).replace(/'/g, '’').replace(/"/g, '\"')}'></ks-m-tile>`)
      })
