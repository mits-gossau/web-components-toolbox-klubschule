  ...
  async renderHTML (fetch) {
    ...
    fetch.then(data => {
      setTimeout(() => {
        ...
        // console.log(data.ppage, this.lastResponsePPage, data.pnext, this.lastResponsePnext)
        if (data.ppage === 1 || (data.ppage === -1 && [-1, 1, 0].includes(this.lastResponsePnext))) this.html = ''
        if (!data) {
