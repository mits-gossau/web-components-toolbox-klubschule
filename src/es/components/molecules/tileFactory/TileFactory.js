  ...
  async renderHTML (detail) {
    ...
    detail.fetch.then(data => {
      ...
      setTimeout(() => {
                ...
                ? /* html */`
                  <m-load-template-tag>
                    <template>
                      ...
                      }'${this.hasAttribute('is-wish-list') ? ' is-wish-list' : ''}${this.hasAttribute('is-info-events') ? ' is-info-events' : ''}${this.isNearbySearch ? ' nearby-search' : ''}${this.hasAttribute('tracking-context') ? ` tracking-context="${this.getAttribute('tracking-context')}"` : ''}>
                      </ks-o-tile-list>
                    </template>
                  </m-load-template-tag>
                `
        ...
        + (!data.courses.length && !detail.onlyfaceted
          ? /* html */`<ks-o-partner-search search-text="${data.searchText}"${data.courses.length ? ' has-courses': ''}${this.hasAttribute('no-partner-search') ? ' no-partner-search' : ''} tab="1"${data.hasSelectedFilter ? ' has-selected-filter' : ''}>
