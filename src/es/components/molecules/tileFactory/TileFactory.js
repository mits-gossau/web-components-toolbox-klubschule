  ...
  async renderHTML (fetch) {
    ...
    fetch.then(data => {
      setTimeout(() => {
        ...
        if (data.ppage === 1 || (data.ppage === -1 && [-1, 1, 0].includes(this.lastResponsePpage)) || (data.courses.length < data.psize && [-1, 1, 0].includes(this.lastResponsePpage)) || data.pskip === data.psize) this.html = ''
        if (!data) {
