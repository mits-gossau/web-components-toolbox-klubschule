...
export default class CourseDialog extends Shadow() {
  ...
  renderDownloads (courseData, courseDetail) {
    // @ts-ignore
    const pdfLink = `${self.Environment.getApiBaseUrl('customer-portal').coursePDF}/?courseType=${courseData.courseType}&courseId=${courseData.courseId}&centerId=${courseData.centerId}&language=${self.Environment.language.substring(0, 2)}`
    return /* html */ `
